...
[![Tests: 152/152](https://img.shields.io/badge/tests-152%2F152-passing-green)]()
...
| 🧪 **152/152 tests** | e2e, graph traversal, gates, router, tools, MCP stdio client, CLI, A2 can_branch availability, S3 並行/快取/prewarm |
...
|---|---|---|
| Functional tests | **152/152 (16 files)** | e2e, graph traversal, gates, router, tools, MCP stdio, CLI, A2 can_branch availability, S3 並行/快取/prewarm |
...
| TS executor dead code | `PipelineExecutor.verifyP0GatesReachable` and the `resolveRoute` import are **never invoked** (the real gate preflight is module-level `verifyP0Reachability`); `parallel_groups` remains decorative at the **graph level** — `executeGraph` drives multi-path via `expand.count` — but Stage 3 now executes its verification tasks in parallel internally (two waves: search → dedup → scrape, v2.1) |
...
├── test/                          # 152 tests across 16 files
