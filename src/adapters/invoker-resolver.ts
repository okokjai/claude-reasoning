...
//
// Production: config.yaml (baseUrl/apiKey/model) with CR_REASONING_API_KEY as
// an env override (spec §6.1/§6.2). Missing config fails loud.
//
...
function readConfig(explicitPath?: string): Record<string, unknown> {
  ...
  if (!existsSync(candidate)) {
    throw new Error(
      `Invoker config not found: ${candidate} (pass --config, set CR_REASONING_CONFIG, or create ./config.yaml)`
    );
  }
...
export async function resolveInvoker(configPath?: string): Promise<LlmInvoker> {
  ...
  const baseUrl = (cfg.baseUrl as string | undefined) ?? process.env.CR_REASONING_BASE_URL;
  if (!baseUrl) {
    throw new Error("Invoker config must define baseUrl (config.yaml or CR_REASONING_BASE_URL)");
  }
  const httpConfig: HttpInvokerConfig = {
    baseUrl,
    apiKey: process.env.CR_REASONING_API_KEY ?? (cfg.apiKey as string | undefined),
    model: process.env.CR_REASONING_MODEL ?? (cfg.model as string | undefined),
  };
