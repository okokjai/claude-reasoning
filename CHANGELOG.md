...
- `scripts/resolve-memory-dir.py` now prefers the canonical memory root (`~/.claude/projects/*/memory`, most-populated root wins) over the deprecated `~/.claude/memory`; sync-check Step 3.5 now validates the live index (all 29 parenthesized refs resolve).
- Plugin-surface honesty audit (docs only): `config.yaml: budget` block annotated as **declared but not enforced**; README Known Limits gains rows for `budget`, `hard_rules` (not configurable at runtime), and TS executor dead code (`verifyP0GatesReachable` + `resolveRoute` never invoked; `parallel_groups` decorative).
- Version bump: skill (SKILL.md), CLI package (package.json), README, release-manifest aligned to 2.0.5. Test count baseline 140 → 143. No public API / CLI / contract schema changes; `can_branch` output semantics unchanged for the real model path.
